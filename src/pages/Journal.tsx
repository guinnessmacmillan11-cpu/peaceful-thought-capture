  ...
  return (
    <div className="min-h-[80vh] px-4 py-6 max-w-md mx-auto pb-24">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <p className="text-muted-foreground text-sm mb-5">Your story, day by day.</p>
      </motion.div>
          ...
          {Array.from({ length: totalDays }).map((_, i) => {
            ...
            return (
              ...
              >
                {day}
                {mood && !isSelected && (
                  <span className="absolute -bottom-0.5 text-[8px]">{moodEmoji[mood.mood]}</span>
                )}
                {hasEntries && !mood && !isSelected && (
                ...
                <div className="space-y-2">
                  {selectedEntries.map((entry) => (
                    <div key={entry.id}>
                      ...
                      >
                        <span className="text-sm mt-0.5">{moodEmoji[entry.mood] || "🌿"}</span>
      ...
      {entries.length > 0 && !selectedDate && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mt-4">
          <p className="text-xs uppercase tracking-widest text-muted-foreground mb-3">Recent conversations</p>
          <div className="space-y-2">
