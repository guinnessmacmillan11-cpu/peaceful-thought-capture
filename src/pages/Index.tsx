...
import { Phone, Heart, Hand, ListChecks, Sparkles, Wind } from "lucide-react";
    ...
    <div className="flex flex-col items-center min-h-[80vh] px-5 pt-10 pb-24 max-w-md mx-auto">
      ...
      >
        {breathing ? (
          <div className="flex flex-col items-center py-2">
            ...
            <motion.p key={breathPhase} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 text-sm text-muted-foreground font-medium">
            ...
            <button onClick={() => { setBreathing(false); setBreathCount(0); }} className="mt-3 text-xs text-muted-foreground underline">Done</button>
          </div>
        ) : (
          <button onClick={() => setBreathing(true)} className="flex items-center gap-4 w-full">
            <div className="w-12 h-12 rounded-full bg-muted flex items-center justify-center shrink-0">
              <Wind size={20} className="text-muted-foreground" />
            </div>
            <div className="text-left">
              <p className="font-heading text-sm">Breathing exercise</p>
              <p className="text-xs text-muted-foreground">4-4-6 calming pattern</p>
            </div>
          </button>
        )}
      </motion.div>
          ...
          <div className="text-center py-2">
            {groundingStep < 5 ? (
              <>
                ...
                <p className="text-sm text-muted-foreground mb-4">Notice {groundingSenses[groundingStep].sense}</p>
                <div className="flex gap-2 justify-center">
                  {groundingStep > 0 && (
                    <button onClick={() => setGroundingStep(groundingStep - 1)} className="text-xs text-muted-foreground underline">Back</button>
                  )}
                  <button onClick={() => setGroundingStep(groundingStep + 1)} className="bg-primary text-primary-foreground px-4 py-1.5 rounded-full text-xs font-medium">
                    Next
                  </button>
                </div>
              </>
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground mb-3">Notice how you feel now.</p>
                <button onClick={() => setGroundingStep(-1)} className="text-xs text-muted-foreground underline">Done</button>
              </>
            )}
          </div>
        ) : (
          <button onClick={() => setGroundingStep(0)} className="flex items-center gap-4 w-full">
            <div className="w-12 h-12 rounded-full bg-muted flex items-center justify-center shrink-0">
              <Hand size={20} className="text-muted-foreground" />
            </div>
            <div className="text-left">
              <p className="font-heading text-sm">5-4-3-2-1 Grounding</p>
              <p className="text-xs text-muted-foreground">Come back to the present</p>
            </div>
          </button>
        )}
      </motion.div>
      ...
      >
        <div className="flex items-center gap-3 mb-3">
          ...
          <p className="font-heading text-sm">Quick relief</p>
        </div>
        <div className="space-y-2">
          {[
            "Splash cold water on your face",
            "Tense all muscles for 5s, then release",
