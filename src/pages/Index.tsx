...
const allAffirmations = [
  ...
  "I radiate positivity and warmth.",
  "I am proud of how far I've come.",
  "Today I choose joy over worry.",
  "I am allowed to rest without guilt.",
  "My mind is clear and focused.",
  "I attract good energy into my life.",
...
function getDailyAffirmations(): string[] {
  ...
  const result: string[] = [];
  for (let i = 0; i < 3; i++) {
    result.push(allAffirmations[(start + i) % allAffirmations.length]);
  }
  return result;
}
...
export default function HomePage() {
  ...
  const handleScrambleCheck = () => {
    ...
    setTimeout(() => {
      ...
      setScrambleInput("");
      if (scrambleIndex < wordScrambles.length - 1) setScrambleIndex((i) => i + 1);
    }, 1200);
  };
  ...
  return (
    <div className="flex flex-col items-center min-h-[80vh] px-5 pt-8 pb-24 max-w-md mx-auto gap-5">
      ...
      {/* Breathing Exercises with Panda */}
                        ...
                        <div className="text-center">
                          ...
                          {colorRound >= colorMaxRounds ? (
                            ...
                            <>
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                {colorOptions.map((c) => (
                                  <motion.button key={c.name} whileTap={{ scale: 0.92 }} onClick={() => handleColorAnswer(c.name)} className={`py-3 rounded-xl ${c.hex} ${c.textColor} font-bold text-sm`}>{c.name}</motion.button>
                                ))}
                              </div>
                      ...
                      {activeGame === "spin" && (
                        <div className="text-center">
                          ...
                          {spinResult ? (
                            ...
                            ><span className="text-3xl">{spinning ? "🌀" : "🎰"}</span></motion.button>
                          )}
                            ...
                            <>
                              <div className="flex justify-center gap-2 mb-4">
                                {wordScrambles[scrambleIndex].scrambled.split("").map((letter, i) => (
                                  <motion.span key={i} initial={{ rotateY: 180 }} animate={{ rotateY: 0 }} className="w-10 h-10 rounded-lg bg-primary/10 border border-primary/30 flex items-center justify-center font-heading text-lg">{letter}</motion.span>
                                ))}
                              </div>
